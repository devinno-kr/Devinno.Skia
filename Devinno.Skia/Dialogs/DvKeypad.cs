...
namespace Devinno.Skia.Dialogs
{
    public class DvKeypad : DvWindow
    {
        ...
        public DvKeypad()
        {
            ...
            tpnl.Columns.Add(new DvTableColumn() { Mode = SizeMode.Percent, Size = 25 });
            tpnl.Columns.Add(new DvTableColumn() { Mode = SizeMode.Percent, Size = 25 });
            tpnl.Columns.Add(new DvTableColumn() { Mode = SizeMode.Percent, Size = 25 });
            tpnl.Columns.Add(new DvTableColumn() { Mode = SizeMode.Percent, Size = 25 });
            ...
            btn0 = new DvButton() { Name = nameof(btn0), Text = "0", Gradient = gr };
            btn1 = new DvButton() { Name = nameof(btn1), Text = "1", Gradient = gr };
            btn2 = new DvButton() { Name = nameof(btn2), Text = "2", Gradient = gr };
            btn3 = new DvButton() { Name = nameof(btn3), Text = "3", Gradient = gr };
            btn4 = new DvButton() { Name = nameof(btn4), Text = "4", Gradient = gr };
            btn5 = new DvButton() { Name = nameof(btn5), Text = "5", Gradient = gr };
            btn6 = new DvButton() { Name = nameof(btn6), Text = "6", Gradient = gr };
            btn7 = new DvButton() { Name = nameof(btn7), Text = "7", Gradient = gr };
            ...
            btnA = new DvButton() { Name = nameof(btnA), Text = "A", Gradient = gr };
            btnB = new DvButton() { Name = nameof(btnB), Text = "B", Gradient = gr };
            btnC = new DvButton() { Name = nameof(btnC), Text = "C", Gradient = gr };
            btnD = new DvButton() { Name = nameof(btnD), Text = "D", Gradient = gr };
            btnE = new DvButton() { Name = nameof(btnE), Text = "E", Gradient = gr };
            btnF = new DvButton() { Name = nameof(btnF), Text = "F", Gradient = gr };
            ...
            btnSign.Drawn += (o, cv) =>
            {
                ...
                var sz = new SKSize(nsz, nsz);
                var rtC = Util.MakeRectangleAlign(bounds, sz, DvContentAlignment.MiddleCenter);
                var rt1 = rtC; rt1.Offset(-nsz / 2, -nsz / 2);
                var rt2 = rtC; rt2.Offset(nsz / 2, nsz / 2);
                var cT = c.ForeColor ?? Design.Theme.ForeColor;
                ...
                Util.DrawIconFA(cv, "fa-plus", nsz, cT, rt1);
                Util.DrawIconFA(cv, "fa-minus", nsz, cT, rt2);
                ...
                using (var p = new SKPaint() { IsAntialias = true })
                {
                    ...
                    cv.DrawLine(rtC.Right, rtC.Top, rtC.Left, rtC.Bottom, p);
                }
            };
            ...
            btnEnter.MouseClick += (o, s) =>
            {
                ...
                else
                {
                    ...
                    if (mode == 0 && ulong.TryParse(sval, out v3)) { Hide(); bOK = true; }
                    else if (mode == 1 && long.TryParse(sval, out v2)) { Hide(); bOK = true; }
                    else if (mode == 2 && decimal.TryParse(sval, out v1)) { Hide(); bOK = true; }
                    else if (mode == 3 && long.TryParse(sval, out v2)) { Hide(); bOK = true; }
            ...
            #endregion
            #region A~F 
            btnA.MouseClick += (o, s) => { sval += "A"; SetText(); };
            btnB.MouseClick += (o, s) => { sval += "B"; SetText(); };
            btnC.MouseClick += (o, s) => { sval += "C"; SetText(); };
            btnD.MouseClick += (o, s) => { sval += "D"; SetText(); };
            btnE.MouseClick += (o, s) => { sval += "E"; SetText(); };
            btnF.MouseClick += (o, s) => { sval += "F"; SetText(); };
            #endregion
            #region Dot
        ...
        void SetText()
        {
            ...
            else if (mode == 2)
            {
                ...
                {
                    if (valueType == typeof(float)) { if (minF1.HasValue && maxF1.HasValue) n = MathTool.Constrain(n, Convert.ToDecimal(minF1.Value), Convert.ToDecimal(maxF1.Value)); }
                    else if (valueType == typeof(double)) { if (minF2.HasValue && maxF2.HasValue) n = MathTool.Constrain(n, Convert.ToDecimal(minF2.Value), Convert.ToDecimal(maxF2.Value)); }
                    else if (valueType == typeof(decimal)) { if (minF3.HasValue && maxF3.HasValue) n = MathTool.Constrain(n, minF3.Value, maxF3.Value); }
            ...
            else if (mode == 1)
            {
                ...
                {
                    if (valueType == typeof(sbyte)) { if (minI8.HasValue && maxI8.HasValue) n = MathTool.Constrain(n, minI8.Value, maxI8.Value); }
                    else if (valueType == typeof(short)) { if (minI16.HasValue && maxI16.HasValue) n = MathTool.Constrain(n, minI16.Value, maxI16.Value); }
                    else if (valueType == typeof(int)) { if (minI32.HasValue && maxI32.HasValue) n = MathTool.Constrain(n, minI32.Value, maxI32.Value); }
                    else if (valueType == typeof(long)) { if (minI64.HasValue && maxI64.HasValue) n = MathTool.Constrain(n, minI64.Value, maxI64.Value); }
            ...
            else if (mode == 0)
            {
                ...
                {
                    if (valueType == typeof(byte)) { if (minU8.HasValue && maxU8.HasValue) n = MathTool.Constrain(n, minU8.Value, maxU8.Value); }
                    else if (valueType == typeof(ushort)) { if (minU16.HasValue && maxU16.HasValue) n = MathTool.Constrain(n, minU16.Value, maxU16.Value); }
                    else if (valueType == typeof(uint)) { if (minU32.HasValue && maxU32.HasValue) n = MathTool.Constrain(n, minU32.Value, maxU32.Value); }
                    else if (valueType == typeof(ulong)) { if (minU64.HasValue && maxU64.HasValue) n = MathTool.Constrain(n, minU64.Value, maxU64.Value); }
