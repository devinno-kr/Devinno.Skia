...
{
    public class DvBarGraph : DvControl
    {
        #region Properties
        #region Color
        ...
        private int DataWH => GraphMode == BarGraphMode.LIST ?
                                            (BarGap + (Series.Count * BarSize) + BarGap) :
                                            (BarGap + BarSize + BarGap);
        #endregion
        #region Scroll
        ...
        #endregion
        #region Direction
        private DvDirectionHV dir = DvDirectionHV.Vertical;
        public DvDirectionHV Direction
        {
            ...
            {
                dir = value;
                if (dir == DvDirectionHV.Vertical) scroll.Direction = ScrollDirection.Horizon;
                else if (dir == DvDirectionHV.Horizon) scroll.Direction = ScrollDirection.Vertical;
            }
        }
        #endregion
        #endregion
        ...
        public DvBarGraph()
        {
            ...
            scroll.GetScrollView = () =>
            {
                ...
                {
                    if (Direction == DvDirectionHV.Horizon) v = Convert.ToInt64(rtGraph.Height);
                    if (Direction == DvDirectionHV.Vertical) v = Convert.ToInt64(rtGraph.Width);
                });
                return v;
            };
        }
        ...
        public void Areas(Action<SKRect, SKRect, SKRect, SKRect, SKRect, SKRect, List<SKSize>> act)
        {
            ...
            using (var p = new SKPaint())
            {
                ...
                {
                    var rMinimum = GraphMode == BarGraphMode.LIST ?
                                GraphDatas.Select(x => x.Values.Min(x2 => x2.Value)).Min() :
                                GraphDatas.Select(x => x.Values.Sum(x2 => x2.Value)).Min();

                    var rMaximum = GraphMode == BarGraphMode.LIST ?
                                GraphDatas.Select(x => x.Values.Max(x2 => x2.Value)).Max() :
                                GraphDatas.Select(x => x.Values.Sum(x2 => x2.Value)).Max();
                ...
                var szName = szNames.OrderByDescending(x => x.Width).FirstOrDefault();
                var szRemark = szRemarks.OrderByDescending(x => x.Width).FirstOrDefault();
                ...
                if (Direction == DvDirectionHV.Horizon)
                {
                    ...
                    lsr.Add(new SizeInfo(DvSizeMode.Pixel, szName.Height));

                    lsc.Add(new SizeInfo(DvSizeMode.Pixel, szName.Width));
                    lsc.Add(new SizeInfo(DvSizeMode.Pixel, GP));
                    lsc.Add(new SizeInfo(DvSizeMode.Percent, 100));
                    lsc.Add(new SizeInfo(DvSizeMode.Pixel, GP));
                    lsc.Add(new SizeInfo(DvSizeMode.Pixel, SCWH));
                    lsc.Add(new SizeInfo(DvSizeMode.Pixel, GP));
                    lsc.Add(new SizeInfo(DvSizeMode.Pixel, rmkW));
                    ...
                    rtGraph = rts[1, 2];
                    rtScroll = rts[1, 4];
                    rtRemark = Util.MakeRectangleAlign(rts[1, 6], new SKSize(rmkW, rmkH), DvContentAlignment.MiddleCenter);
                    #endregion
                }
