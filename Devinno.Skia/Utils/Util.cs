...
namespace Devinno.Skia.Utils
{
    public class Util
    {
        ...
        #region MeasureText
        public static SKSize MeasureText(string text, string font, int size, DvFontStyle style)
        {
        ...
        #region DrawText
        /*
        public static void DrawText(SKCanvas canvas, string text, SKRect rect, DvContentAlignment align, SKPaint paint)
        {
            if (!string.IsNullOrWhiteSpace(text))
            {
                ...
                foreach (var v in ls)
                {
                    ...
                    switch (align)
                    {
                        case DvContentAlignment.TopLeft: xText = rtv.Left - rtText.Left; yText = rtv.Top - rtText.Top; break;
                        case DvContentAlignment.TopCenter: xText = rtv.MidX - rtText.MidX; yText = rtv.Top - rtText.Top; break;
                        case DvContentAlignment.TopRight: xText = rtv.Right - rtText.Right; yText = rtv.Top - rtText.Top; break;

                        case DvContentAlignment.MiddleLeft: xText = rtv.Left - rtText.Left; yText = rtv.MidY - rtText.MidY; break;
                        case DvContentAlignment.MiddleCenter: xText = rtv.MidX - rtText.MidX; yText = rtv.MidY - rtText.MidY; break;
                        case DvContentAlignment.MiddleRight: xText = rtv.Right - rtText.Right; yText = rtv.MidY - rtText.MidY; break;

                        case DvContentAlignment.BottomLeft: xText = rtv.Left - rtText.Left; yText = rtv.Bottom - rtText.Bottom; break;
                        case DvContentAlignment.BottomCenter: xText = rtv.MidX - rtText.MidX; yText = rtv.Bottom - rtText.Bottom; break;
                        case DvContentAlignment.BottomRight: xText = rtv.Right - rtText.Right; yText = rtv.Bottom - rtText.Bottom; break;
                    }
        ...
        */
        #endregion
        #region DrawText
        public static void DrawText(SKCanvas canvas, string text, string font, int size, DvFontStyle style, SKColor color, SKRect rect, DvContentAlignment align = DvContentAlignment.MiddleCenter, bool shadow = true, byte? shadowAlpha = null)
        {
            if (!string.IsNullOrWhiteSpace(text))
            {
                ...
                using (var paint = new SKPaint { IsAntialias = DvDesign.AA, SubpixelText = true })
                {
                    ...
                    if (shadow)
                    {
                        if (DvDesign.ActiveDesign.Theme.Brightness == ThemeBrightness.Light)
                        {
                            fOut = SKImageFilter.CreateDropShadow(0, 1, 0, 0, Util.FromArgb(120, SKColors.White));
                            paint.ImageFilter = fOut;
                        }
                        else
                        {
                            fOut = SKImageFilter.CreateDropShadow(1, 1, 0, 0, Util.FromArgb(80, SKColors.Black));
                            paint.ImageFilter = fOut;
                        }
                    }
        ...
        #endregion
        #region DrawIconFA
        public static void DrawIconFA(SKCanvas canvas, string iconString, int size, SKColor color, SKRect rect, DvContentAlignment align = DvContentAlignment.MiddleCenter, bool shadow = true, byte? shadowAlpha = null)
        {
        ...
        #endregion
        #region DrawTextIcon
        public static void DrawTextIcon(SKCanvas canvas, string text, string font, int textsize, DvFontStyle style, int gap, string iconString, int iconSize, SKColor color, SKRect rt, DvTextIconAlignment talign = DvTextIconAlignment.LeftRight, DvContentAlignment align = DvContentAlignment.MiddleCenter, bool shaodw = true, byte shadowAlpha = 80)
        {
            var fai = FA.GetFAI(iconString);
            if (fai == null)
            {
                DrawText(canvas, text, font, textsize, style, color, rt, align, shaodw, shadowAlpha);
            }
            else
            {
                var r = fai;

                var rtmFA = Util.FromRect(0, 0, iconSize + 2, iconSize + 2);

                TextIconBounds(text, font, textsize, style, gap, rt, rtmFA, talign, align, (rtFA, rtTX) =>
                {

                    DrawIconFA(canvas, iconString, iconSize, color, rtFA, DvContentAlignment.MiddleCenter, shaodw, shadowAlpha);
                    DrawText(canvas, text, font, textsize, style, color, rtTX, DvContentAlignment.MiddleCenter, shaodw, shadowAlpha);
        ...
        public static SKSize MeasureTextIcon(string text, string font, int textsize, DvFontStyle style, int gap, string iconString, int iconSize, DvTextIconAlignment talign)
        {
            SKSize ret = new SKSize(0F, 0F);
        ...
        public static void TextIconBounds(string text, string font, int textsize, DvFontStyle style, float gap, SKRect rt, SKRect rtIcon, DvTextIconAlignment talign, DvContentAlignment align, Action<SKRect, SKRect> act)
        {
            using (var paint = new SKPaint())
            {
                if (string.IsNullOrEmpty(text))
                {
                    act(Util.MakeRectangle(rt, new SKSize(Convert.ToInt32(rtIcon.Width), Convert.ToInt32(rtIcon.Height))), Util.FromRect(0, 0, 0, 0));
                }
                else
                {
                    paint.Typeface = FontTool.GetFont(font, style);
